...
class TestPOSInvoice(unittest.TestCase):
	...
	def test_get_item_group(self):
		...
		if not frappe.db.exists("Item Group", "Test Group A"):
			frappe.get_doc(
				{
					"doctype": "Item Group",
					"item_group_name": "Test Group A",
					"parent_item_group": "All Item Groups",
					"is_group": 1,
				}
			).insert(ignore_permissions=True)

		if not frappe.db.exists("Item Group", "Child Group A1"):
			frappe.get_doc(
				{
					"doctype": "Item Group",
					"item_group_name": "Child Group A1",
					"parent_item_group": "Test Group A",
					"is_group": 0,
				}
			).insert(ignore_permissions=True)
