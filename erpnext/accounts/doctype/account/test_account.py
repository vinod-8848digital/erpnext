...
class TestAccount(unittest.TestCase):
	...
	def test_validate_parent_child_account_type_TC_ACC_168(self):
		...
		with self.assertRaises(
			frappe.ValidationError, msg="Only Parent can be of type {0}".format(acc.account_type)
		):
	...
	def test_validate_parent_account_not_assign_TC_ACC_169(self):
		...
		with self.assertRaises(
			...
			msg="Account {0}: You can not assign itself as parent account".format(account.parent_account),
		):
	...
	def test_validate_parent_account_is_group_TC_ACC_170(self):
		...
		with self.assertRaises(
			frappe.ValidationError,
			msg="Account Test Account - _TC: Parent account {0} can not be a ledger".format(
	...
	def test_validate_parent_account_for_company_TC_ACC_171(self):
		...
		with self.assertRaises(
			frappe.ValidationError,
			msg="Account Test Account - _TC: Parent account {0} does not belong to company: _Test Company".format(
				account.parent_account
			),
		):
			account.save(ignore_permissions=True)
	...
	def test_validate_existing_bank_account_TC_ACC_192(self):
		# Setup
