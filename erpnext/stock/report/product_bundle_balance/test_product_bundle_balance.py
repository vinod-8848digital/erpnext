...
class TestProductBundleBalance(FrappeTestCase):
	def setUp(self):
		...
		# Child items
		self.child_item_1 = create_item(item_code="BUNDLE-CHILD-1", is_stock_item=1, company=self.company, valuation_rate = 100)
		self.child_item_2 = create_item(item_code="BUNDLE-CHILD-2", is_stock_item=1, company=self.company, valuation_rate = 100)

		# Parent (bundle) item
		self.parent_item = create_item(item_code="_Test Product Bundle Item New", is_stock_item=0, company=self.company)


		# make_item("_Packed Item New 1", {"is_stock_item": 1})
		make_product_bundle("_Test Product Bundle Item New", ["BUNDLE-CHILD-1", "BUNDLE-CHILD-2"], 2)
		# make_product_bundle("_Test Product Bundle Item New", ["BUNDLE-CHILD-2"], 2)

		# self.bundle_doc = make_product_bundle(
		#     new_item_code=self.parent_item.name,
		#     items=[
		#         {"item_code": self.child_item_1.name, "qty": 2, "uom": self.child_item_1.stock_uom},
		#         {"item_code": self.child_item_2.name, "qty": 3, "uom": self.child_item_2.stock_uom},
		#     ],
	...
	def test_bundle_balance_report(self):
		filters = {
			"company": self.company,
		...
		# Parent row check
		parent_rows = [row for row in data if row.get("indent") == 0 and row.get("item_code") == self.parent_item.name]
		...
		# Child rows check
		child_rows = [row for row in data if row.get("indent") == 1 and row.get("parent_item") == self.parent_item.name]
