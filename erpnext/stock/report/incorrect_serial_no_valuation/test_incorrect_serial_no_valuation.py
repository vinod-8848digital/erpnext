...
class TestIncorrectSerialNoValuationReport(unittest.TestCase):
	...
	def test_empty_filters_returns_balance_row_only(self):
		# Run report with empty filters: no item code, no date filters
		columns, data = report_execute({})
		# Should return only the balance row (last row)
	...
	def test_balanced_serial_no_excluded(self):
		serial_no = "SN-BAL-001"
		# Create two separate stock entries to simulate IN and OUT
		in_voucher = self._make_stock_entry(serial_no, qty=1, rate=100, voucher_no="TEST-SN-BAL-IN")
	...
	def test_negative_qty_triggers_incorrect(self):
	...
	def test_multiple_serial_nos(self):
		...
		# Delete SLE for serial_no_2 and re-insert manually with incorrect valuation
	...
	def test_missing_serial_no_is_ignored(self):
		voucher_no = self._make_stock_entry(serial_no="", qty=1, rate=100, voucher_no="TEST-SN-MISSING")
		columns, data = report_execute({"item_code": self.item.name})
		serials = [row.get("serial_no") for row in data]
		self.assertNotIn("", serials)

	def test_unsubmitted_stock_entry_is_ignored(self):
		serial_no = "SN-UNSUB"
		se = frappe.get_doc(
			{
				"doctype": "Stock Entry",
